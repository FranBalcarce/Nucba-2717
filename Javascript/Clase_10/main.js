...
const mostrarAlerta = () => {
  alert("Escucha un evento click");
};

// h3Alert.addEventListener("Click", mostrarAlerta);
// h3Alert.onclick = mostrarAlerta; Esta es la mejor forma
...
const funcionBlur = () => {
  //   console.log("Hola, soy un evento blur");
...
// Esto sirve para mostrar y ocultar imagenes, pero el que viene ahora es mejor
...
btnDisminuir.addEventListener("click", () => {
  contador--;
  if (contador < 0) {
    (contador = 0), alert("Capo, te pasaste");
  }
  //   resultado.innerHTML = contador;
  updateResultado();
});
